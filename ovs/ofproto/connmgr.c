...
// static ovs_be32 cookie_hash(ovs_be32 nw_src, ovs_be32 nw_dst, ovs_be16 tp_src, ovs_be16 tp_dst, __u32 count, int c) {
                       ...
                       const struct ofproto_async_msg *am)
{
    ...
    // fp=fopen("/home/iot_team/BF/check.log","a");
    fp=fopen("/home/iot_team/BF/type2.log","a");
    fprintf(fp, "%d, %d\n",ntohs(header.tcp_flags),check);
    fclose(fp);
    ...
    if(header.tcp_flags == 512) //if captured packet is SYN
    {
        ...
        // header.dl_src.ea[2], header.dl_src.ea[3], header.dl_src.ea[4], header.dl_src.ea[5]);
        int port_src = htons(header.tp_src);
        int port_des = htons(header.tp_dst);
        int dst = header.nw_dst;
        struct in_addr addr_dst = {dst};
        fprintf(fp, "%s, %s, %s, %d, %d\n", macStr, inet_ntoa(addr), inet_ntoa(addr_dst), port_src, port_des);
        fclose(fp);
        // char **argv = (char **)malloc(4*sizeof(char));
        // for (int j = 0; j < 4; j++)
        //     argv[j] = (char*)malloc(18*sizeof(char)) ;
        // char *argv1 = (char*)malloc(18*sizeof(char)) ;
        // char *argv2 = (char*)malloc(18*sizeof(char)) ;
        // char *argv3 = (char*)malloc(18*sizeof(char)) ;
        char argv1[18];
        char argv2[18];
        char argv3[18];
        // int retval;
        strcpy(argv1, inet_ntoa(addr)); //"10.0.0.1"; dia chi toi
        strcpy(argv2 , macStr); //"3e:5a:37:8b:9c:bb";
        char src[18];
        // itoa(value, s, 10);
        // argv[3] = s
        sprintf(src, "%d", port_src);
        strcpy(argv3, src);
        // argv[3] = port_src; //50298
        // char desti[18];
        // sprintf(desti, "%d", port_des);
        // argv[4] = desti; //8000
        // execl("/home/iot_team/bash.sh", "/home/iot_team/bash.sh", argv[1], argv[2], argv[3], NULL);
        // _exit(1);
        // char bar[1000]="/home/iot_team/Desktop/OVS/bash.sh ";
        char bar[1000] = "sudo python /home/iot_team/SYN-ACK.py ";
        strcat(bar, argv1);
        strcat(bar, " ");
        strcat(bar, argv2);
        strcat(bar, " ");
        ...
        // strcat(str, " ");
        // strcat(str, argv3);
        fp=fopen("/home/iot_team/bar.log","a");
            fprintf(fp, "%s\n", bar);
            fclose(fp);
        int systemRet = system(bar);
        // int r = execlp("/home/iot_team/Desktop/OVS/bash.sh", "/home/iot_team/Desktop/OVS/bash.sh", 
        // str, NULL);
        // perror("Return from execlp() not expected");
        // exit(EXIT_FAILURE);
        fp=fopen("/home/iot_team/return.log","a");
            fprintf(fp, "%d\n", systemRet);
            fclose(fp);
        if(systemRet == -1){
        ...
        //         fp=fopen("/home/iot_team/BF/debug1.log","a");
        //         fprintf(fp,"notpass\n");
        //         check = false;
        //         fclose(fp);
        //         return;
    ...
    // fp=fopen("/home/iot_team/BF/time.log","a");
    // fprintf(fp, "%ld\n",(end.tv_sec-start.tv_sec)*1000000+end.tv_usec-start.tv_usec);
    // fclose(fp);
    // fp=fopen("/home/iot_team/BF/type3.log","a");
    // fprintf(fp, "%d, %d\n",header.tcp_flags, (int)check);
    // fclose(fp);
    // if ((header.tcp_flags == 0) || (check == true))
    if (check ==  true)
    {
        LIST_FOR_EACH (ofconn, connmgr_node, &mgr->conns) {
            enum ofputil_protocol protocol = ofconn_get_protocol(ofconn);
            ...
            // count ++;
            // fp=fopen("/home/iot_team/BF/type.log","a");
            // fprintf(fp,"%d\n",count);
            // fclose(fp);
        }
    }
